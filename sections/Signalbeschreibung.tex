\section{Signalbeschreibung \skript{1}}
	...
	von der Zeit t ab) und können nur durch \textbf{Wahrscheinlichkeitsverteilungen} beschrieben werden.
		\subsubsection{Energie- und Leistungssignale \skript{3}}
			...
			\begin{tabularx}{\textwidth}{|p{2.9cm}|c|c|c|}
			\hline
				{}
	...
	\subsection{Korrelation \skript{8}}	
		Mittels Vergleichen von Funktionen mit sich selbst oder mit einer anderen Funktion, kann herausgefunden werden, ob eine Korrelation vorliegt oder nicht. \\
		\textbf{Hinweis:} Korrelation $\neq$ Kausalität, aber Kausalität $=$ Korrelation
		
		\begin{itemize}
			\item Autokorrelation: Vergleich mit sich selbst.
			\item Kreuzkorrelation: Vergleich mit einer anderen Funktion.
		\end{itemize}
		
		\subsubsection{Autokorrelationsfunktion (AKF) \skript{8}} 
		...
		\subsubsection{Kreuzkorrelationsfunktion (KKF) \skript{11}} 
	...
	\subsection{Amplitudenanalyse von Signalen \skript{29}}
		...
		& 	\begin{minipage}[]{11cm}
			...
			''\textit{Zeit während sich Signal in bestimmtem Amplitudenintervall aufhält}''\\ \newline
		...
		& 	\begin{minipage}[]{11cm}
				...
				\begin{itemize}
					\item Es gibt keine negativen Werte! $p(a) \geq 0 \ \ \forall a$
     				\item Gesamtwahrscheinlichkeit:\enspace \fbox{$\int\limits_{-\infty}^{\infty} p(a) da = 1$}
     				\item Wahrscheinlichkeit ${a_1 < a < a2}$:\enspace \fbox{$p(a_1 < a < a_2) = \int\limits_{a_1}^{a_2} p(a) da$}
   				\end{itemize}
      		\end{minipage}
		\end{tabular}\\ \\
	...
	\subsection{Stochastische Signale und deren Verteilungen \skript{38}}
		...
		\begin{minipage}[]{22.5cm}
			...
			\\ \hline
			\end{tabular}\\ \\
			\textbf{Stochastische Signale} sind schwach stationär (Linearer Mittelwert $x_0$ und Autokorrelation $\phi{xx}(t)$ hängen nicht von der Zeit t ab) und können nur durch Wahrscheinlichkeitsverteilungen beschrieben werden.
			\\
			...
			\subsubsection{Addition von zweier stochastischen Signalen}
				\begin{multicols}{2}
					Die Addition von zwei unabhängigen Signalen entspricht der Faltung ihrer Wahrscheinlichkeitsverteilungen.
					\vfill\null
					\columnbreak
					\vfill\null
					\fbox{$x_1(t) : p_1(a)$} \raisebox{.5pt}{\textcircled{\raisebox{-.9pt} {$+$}}} \fbox{$x_2(t) : p_2(a)$} $=$ 
					\fbox{$x(t) : p(a) = p_1(a) * p_2(a)$}
				\end{multicols}
		\end{minipage}
		}
